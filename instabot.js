const options = {
  ...
  include:
    "인스타, 인친, 인친해요, 라이크, 좋아요, 좋아요환영, 좋아요반사, 라이크반사, 반사, l4l, like, instalike, follow, followme, 맞팔, 팔로우, 맞팔해요, f4f",
  exclude:
    "10k, 20k, 30k, 10kfollowers, 20kfollowers, 맞팔100, 선팔100, 오늘의훈남, 훈남그램, 흔남그램, 육아스타그램, 육아, 육아그램, 고딩, 18, 19, 고1, 고2, 고3",
  comments: {
    conditions: {
      followback:
        "선팔맞팔, 선팔하면맞팔, 선팔하면맞팔가요, 선팔후맞팔, 맞팔, 소통, 언팔싫어요, 언팔하면언팔",
    ...
    emoji:
      "😊, 😛, 🤗, 🤗🤗, 😄, 😄😄, 🤙, 👍, 🙌, 🙌🙌, 🙏, 🙏🙏🙏, :), :D, ;), ㅎㅎ, ㅋㅋㅋ,•̀.̫•́✧,٩(๑òωó๑)۶,(๑´•.̫•`๑),(•̀ㅁ•́), (｀･ω･´)ゞ, ",
  },
};
class Instabot {
  constructor(options) {
    ...
    this.element = {
      ...
      followersBtn: `a[href*="${window.location.pathname}followers"]`,
      followersOverHidden: "svg[aria-label='Loading...']._abdx",
      followersInnerHeight: "._aano>div>div",
    ...
    this.comments = {
      conditions: {
        followback: options.comments.conditions.followback
          .replace(/ /g, "")
          .split(","),
        likeback: options.comments.conditions.likeback
          .replace(/ /g, "")
          .split(","),
      },
      comments: {
    ...
    this.font = {
      ...
      super:
        "font-size:12px;font-weight:bold;text-align:center;background:black;",
      small: "font-size:8px;",
  ...
  init() {
    ...
    this.delay(this.time.delayInitial).then(() => this.analyzePost());
  }
  archive() {
    if (this.status.liked.length > 0) {
      this.status.archivedLiked = [
        ...this.status.liked,
        ...this.status.archivedLiked,
      ];
  ...
  openPost() {
    if (!document.querySelector(this.element.popup)) {
      const post = this.options.includeTop
        ...
        ? document.querySelector(this.element.recentPost)
        : null;
      post ? post.click() : this.logger(`Post missing`, this.font.error);
    }
  }
  analyzePost() {
    if (this.status.inProgress) {
      ...
      this.resetPost()
        ...
        .then(
          () => this.processPost(),
          (err) => this.logger(err, this.font.error),
        )
  ...
  nextImage() {
    if (this.status.liked.length >= this.conditions.maxLiked) {
      this.logger(
        `You have already liked ${this.conditions.maxLiked} images. Restarting will reset`,
        this.font.heading,
      );
    ...
    if (this.status.followed.length >= this.conditions.maxFollows) {
      this.logger(
        `You have already followed ${this.conditions.maxFollows} followers. Restarting will reset`,
        this.font.heading,
      );
      const evt = new KeyboardEvent("keydown", { keyCode: 222, which: 222 });
      document.dispatchEvent(evt);
    ...
    if (this.status.inProgress) {
      ...
      this.logger(
        `Liked ${this.status.liked.length} images. (max:${this.conditions.maxLiked})`,
        this.font.small,
      );
      this.logger(
        `Followed ${this.status.followed.length} people (max:${this.conditions.maxFollows})`,
        this.font.small,
      );
      this.logger(
        `Time remaining: ${
          Math.round(
            ((this.time.maxDuration - (performance.now() - this.time.start)) /
              this.min) *
              10,
          ) / 10
        } minutes`,
      ...
      this.logger(
        `Total Like count: ${this.status.liked.length} images`,
        this.font.pass,
      );
      this.logger(
        `Total Follow count: ${this.status.followed.length}`,
        this.font.pass,
      );
      this.status.followed.forEach((f) => {
        this.logger(
          `<a style="${this.font.link}" target="_blank" href="${f.person.personLink}">${f.person.personName}</a> `,
          this.font.small,
        );
      });
  ...
  getName() {
    return new Promise((resolve) => {
      this.post.person =
        document.querySelector(this.element.name) != null
          ? {
          ...
          : this.logger(`Couldn't load name. Retrying...`, this.font.error) &&
            this.waitFor(delay, () =>
              document.querySelector(this.element.name) != null
                ? {
                    personName: document.querySelector(this.element.name)
                      .innerText,
                    personLink: document.querySelector(this.element.name).href,
                    personImage: document.querySelector(
                      this.element.personImage,
                    ).src,
                  }
                : null,
            );
  ...
  getTags() {
    return new Promise((resolve) => {
      ...
      const hasTag =
        this.conditions.include.length > 0
          ? Array.from(tags)
              .filter(function (w) {
          ...
          : [];
      const hasExcludes =
        this.conditions.exclude.length > 0
          ? Array.from(tags)
              .filter(function (w) {
                return this.indexOf(w.innerText.replace("#", "")) >= 0;
          ...
          : [];
      const hasF4F =
        this.comments.conditions.followback.length > 0
          ? Array.from(tags).filter(function (w) {
              return this.indexOf(w.innerText.replace("#", "")) >= 0;
            }, this.comments.conditions.followback)
          : [];
      const hasL4L =
        this.comments.conditions.likeback.length > 0
          ? Array.from(tags).filter(function (w) {
              return this.indexOf(w.innerText.replace("#", "")) >= 0;
  ...
  getNumberOfLikes() {
    return new Promise((resolve) => {
      this.post.numberOfLikes =
        document.querySelector(this.element.numberOfLikes) != null
          ? parseInt(
              document
                .querySelector(this.element.numberOfLikes)
                .innerText.replace(",", ""),
            )
          : 0;
      return resolve();
  ...
  getImage() {
    return new Promise((resolve) => {
      ...
      const isSafe =
        alt != null && this.conditions.imageAlt.length > 0
  ...
  validatePost() {
    return new Promise((resolve, reject) => {
      const { person, src, tags, numberOfLikes, liked, comment, followed } =
        this.post;
      ...
      if (person == null) {
        ...
        this.logger(
          `ID: <a style="${this.font.link}" target="_blank" href="${person.personLink}">${person.personName}</a>`,
          this.font.heading,
        );
        this.logger(
          `Current post: <a style="${this.font.link}" target="_blank" href="${src}">${src}</a>`,
          this.font.small,
        );
      }
      ...
      if (numberOfLikes < this.conditions.minLikes) {
        ...
        this.logger(
          `Passed like filter : ${this.conditions.minLikes} < ${numberOfLikes} < ${this.conditions.maxLikes}`,
          this.font.good,
        );
      }
      ...
      if (tags.hasTag.length == 0) {
        ...
        this.logger(
          `Found ${tags.hasTag.length} matching tags:${wantedTags}`,
          this.font.good,
        );
      }
      return resolve();
    ...
    return new Promise((resolve) => {
      if (this.status.inProgress) {
        ...
        this.likePost()
          ...
          .then(
            () => this.follow(),
            (err) => (err ? this.logger(err, this.font.error) : null),
          )
          .then(() => resolve());
      } else {
  ...
  likePost() {
    return new Promise((resolve) => {
      this.logger(
        `....liking post in ${this.time.delayLike / this.s}s..`,
        this.font.progress,
      );
      this.delay(this.time.delayLike).then(() => {
  ...
  writeComment() {
    return new Promise((resolve, reject) => {
      const { liked, tags, image } = this.post;
      ...
      if (
        ...
        if (input) {
          ...
          if (comment != null) {
            ...
            return reject(
              "Comments generating failed : No matching tags for comments",
            );
          }
        } else {
    ...
    return new Promise((resolve, reject) => {
      if (
        ...
        if (btn) {
          ...
          this.logger(
            `....posting comment in ${delayComment}s..`,
            this.font.progress,
          );
          this.delay(delayComment * this.s).then(() => {
            btn.click();
            this.logger(
              `....checking if comment posted ${
                this.time.delayNext / this.s
              }s...`,
              this.font.progress,
            );
            setTimeout(() => {
              const hasFailed = document.querySelector(
                this.element.commentFailAlert,
              )
                ? true
                : false;
              if (hasFailed) {
                ...
                this.logger(
                  `Successfully POSTED comment: "${this.post.comment}"`,
                  this.font.pass,
                );
                return resolve();
              }
  ...
  follow() {
    return new Promise((resolve) => {
      const { tags, comment, followbtn, followed } = this.post;
      ...
      if (
        ...
        this.logger(
          `....Following in ${this.time.delayFollow / this.s}s....`,
          this.font.progress,
        );
        this.delay(this.time.delayFollow).then(() => {
          if (followbtn) {
            followbtn.click();
            this.post.followed = true;
            this.status.followed.push(this.post);
            this.logger(
              `FOLLOWED: <a style="${this.font.link}" style="${this.font.link}" target="_blank" href="${this.post.person.personLink}">${this.post.person.personLink}</a>`,
              this.font.pass,
            );
            resolve();
          }
        ...
        !this.options.isFiltering
          ...
          : this.conditions.maxFollows <= this.status.followed.length
          ? this.logger(
              `Skipping follow. Maximum follows reached`,
              this.font.error,
            )
          : !this.options.isFollowing
          ...
          : !tags.hasF4F
          ? this.logger(
              `Skipping follow. Missing required tags.`,
              this.font.error,
            )
          : null;
        return resolve();
  ...
  async getUnfollowers() {
    ...
    const followersBtn =
      document.querySelectorAll(this.element.followersBtn).length > 0;
    if (followersBtn) {
      const following = await this.loadFollowers(true).catch(
        (err) => this.logger(err, this.font.error) && false,
      );
      const followers = following
        ? await this.loadFollowers(false).catch(
            (err) => this.logger(err, this.font.error) && false,
          )
        : null;
      if (following && followers) {
        ...
        this.logger(
          `You have ${unfollowers.length} people who are not following back:`,
          this.font.heading,
        );
        unfollowers.forEach((f) => {
          this.logger(
            `${f} : <a style="${this.font.link}" href="https://www.instagram.com/${f}" target="_blank">https://www.instagram.com/${f}</a>`,
            this.font.small,
          );
        });
        return unfollowers;
  ...
  async loadFollowers(loadingFollowings) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      loadingFollowings ? flBtn[flBtn.length - 1].click() : flBtn[0].click();
      const result = await this.delay(this.time.delayInitial)
    ...
    return new Promise((resolve, reject) => {
      const itvl = setInterval(() => {
        ...
        const innerbox = document.querySelector(
          this.element.followersInnerHeight,
        );
        if (scroll && innerbox) {
          const loadedHeight = innerbox.scrollHeight;
          const isLoading = document.querySelector(
            this.element.suggestionsTitle,
          )
            ? scroll.scrollTop != loadedHeight
            : document.querySelector(this.element.followerPopupLoadingIcon)
            ? true
          ...
          if (isLoading) {
            ...
            const list = Array.from(
              document.querySelectorAll(this.element.followersList),
            ).map((p) => {
              return p.innerText;
            });
  ...
  toggleFilter() {
    ...
    this.options.isFiltering
      ? this.logger(
          `Filtering turned ON. Instabot will process tags`,
          this.font.pass,
        )
      : this.logger(
          `Filtering turned OFF. Instabot will like everything but will not comment / follow`,
          this.font.error,
        );
  }
  toggleFollowing() {
    ...
    this.options.isFollowing
      ? this.logger(
          `Following ON. Instabot is following after commenting for F4F`,
          this.font.pass,
        )
      : this.logger(
          `Following OFF. Instabot is not commenting for F4F / following`,
          this.font.error,
        );
  }
  toggleCommenting() {
    ...
    this.options.isCommenting
      ? this.logger(
          `Commenting ON. Instabot is commenting for L4L and F4F`,
          this.font.pass,
        )
      : this.logger(
          `Commenting OFF. Instabot is not commenting / following`,
          this.font.error,
        );
  }
}
...
class InstabotUI {
  constructor(instabot) {
    ...
    this.style = {
      ...
      logger:
        "position:fixed;top:50px;left:0;width:25%;height:500px;overflow-y:auto;padding:15px;color:white;background:#252525;z-index:99;border-radius:5px;resize:vertical;box-shadow: 0 10px 20px rgba(0,0,0,0.19),0 6px 6px rgba(0,0,0,0.23);",
      popup:
        "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:80%;height:80%;padding:10px;border-radius:15px;z-index:99;background:white;box-shadow: 0 10px 20px rgba(0,0,0,0.19),0 6px 6px rgba(0,0,0,0.23);",
      smallpopup:
        "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:40%;height:80%;padding:10px;border-radius:15px;z-index:99;background:white;box-shadow: 0 10px 20px rgba(0,0,0,0.19),0 6px 6px rgba(0,0,0,0.23);",
      popupInner: "overflow:auto;width:100%;height:100%",
      popupClose:
        "position:fixed; right:2em; top:1em;border:none;border-radius:5px;color:white;background:black;",
      ...
      person:
        "display:flex;justify-content:space-between;align-items:center;flex-flow:row;width:100%;margin-bottom:1em;",
      ...
      postImage:
        "width:100%;min-width:30px;min-height:30px;background:lightgrey;height:auto;",
      ...
      btn: {
        ...
        green:
          "background:teal;color:white;outline:none;border-radius:5px;border:none;padding:5px 10px;margin-top:5px;",
      },
    };
  }
  init() {
                ...
                <div style="padding:1em;">
                    ...
                    ${
                      b.options.isFiltering
                        ...
                        <p style="margin-bottom:5px;">${
                          b.conditions.maxLiked
                        }</p>
                        <h3 style="${this.style.subheading}">Likes max :</h3>
                        <p style="margin-bottom:5px;">${
                          b.conditions.maxLikes
                        }</p>
                        <h3 style="${this.style.subheading}">Likes min :</h3>
                        <p style="margin-bottom:5px;">${
                          b.conditions.minLikes
                        }</p>
                        <h3 style="${this.style.subheading}">Like if :</h3>
                        <p style="margin-bottom:5px;">${b.conditions.include.join(
                          ",",
                        )}</p>
                        <h3 style="${
                          this.style.subheading
                        }">Don't like if :</h3>
                        <p style="margin-bottom:5px;">${b.conditions.exclude.join(
                          ",",
                        )}</p>
                        <h3 style="${
                          this.style.subheading
                        }">Image type required to comment  :</h3>
                        <p style="margin-bottom:5px;">${b.conditions.imageAlt.join(
                          ",",
                        )}</p>
                        <h3 style="${this.style.subheading}">Comment if :</h3>
                        <p style="margin-bottom:5px;">${
                          b.comments.conditions.likeback.join(",") +
                          "," +
                          b.comments.conditions.followback.join(",")
                        }</p>
                    `
                        : `
                    ...
                    <h2 style="${
                      this.style.heading
                    }margin-top:1em;">Results</h2>
                    <h3 style="${this.style.subheading}">Current liked:</h3>
                    <p style="margin-bottom:5px;">${
                      b.status.liked.length + b.status.archivedLiked.length
                    }</p>
                    <h3 style="${this.style.subheading}">Current followed:</h3>
                    <p style="margin-bottom:5px;">${
                      b.status.followed.length +
                      b.status.archivedFollowed.length
                    }</p>
                    ${
                      b.status.archivedFollowed.length > 0
                        ? `${b.status.archivedFollowed.map(
                            (f) =>
  ...
  myLikedBtnClicked(btn) {
    ...
    if (btn.className == "off") {
      ...
      const liked = [
        ...this.instabot.status.liked,
        ...this.instabot.status.archivedLiked,
      ];
      const html =
        liked.length > 0
                      ...
                      .map(
                        (t) =>
                          `<li style="${this.style.li}">
                            <a style="${this.style.person}" href="${
                            t.person.personLink
                          }" target="_blank">
                                <span style=${
                                  t.followed
                                    ? this.style.personFollowed
                                    : this.style.personNotFollowed
                                }>${
                            t.followed ? "Followed" : "Not Following"
                          }</span>
                                <p>${t.person.personName}</p>
                                <img style="${this.style.personImage}" src="${
                            t.person.personImage
                          }"/>
                            </a>
                            <a style="text-align:center;"href="${
                              t.src
                            }"target="_blank">
                                <img style="${this.style.postImage}" src="${
                            t.image.src
                          }"/>
                            </a>
                            <p style="${this.style.postCommented}">${
                            t.comment != null
                              ? `Comment: ${t.comment}`
                              : "Not commented"
                          }</p>
